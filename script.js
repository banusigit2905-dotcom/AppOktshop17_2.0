...
// --- VERIFIKASI 2 LANGKAH (2FA) KHUSUS LOGIN ADMIN — KODE OTP VIA EMAIL ---
// Pakai kode OTP 6 digit yang dikirim ke email (via EmailJS, sama seperti OTP tukar poin).
//
...
function sendAdminLoginOtp() {
    currentAdminLoginOTP = String(Math.floor(100000 + Math.random() * 900000));

    const statusEl = document.getElementById("adminOtpStatusText");
    const resendBtn = document.getElementById("btnResendAdminOtp");
    const targetEmail = ADMIN_OTP_EMAIL_OVERRIDE || currentUser.email;
    const emailMasked = maskEmail(targetEmail);

    if (typeof emailjs === 'undefined') {
        statusEl.innerHTML = `⚠️ Layanan email belum aktif (EmailJS belum terpasang). Hubungi developer.`;
        return;
    }

    statusEl.innerText = "Mengirim kode verifikasi ke email admin...";
    if (resendBtn) resendBtn.disabled = true;

    emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, {
        to_email: targetEmail,
        to_name: currentUser.nama,
        otp_code: currentAdminLoginOTP
    }).then(() => {
        statusEl.innerHTML = `✅ Kode verifikasi sudah dikirim ke <b>${emailMasked}</b>. Cek inbox (atau folder spam), lalu masukkan kodenya di bawah.`;
        if (resendBtn) resendBtn.disabled = false;
    }).catch((err) => {
        statusEl.innerHTML = `❌ Gagal mengirim email verifikasi. Coba "Kirim Ulang Kode".`;
        console.log("EmailJS error (admin OTP):", err);
        if (resendBtn) resendBtn.disabled = false;
    });
}
...
document.getElementById("formAdminOtpVerify").onsubmit = (e) => {
    ...
    if (!currentAdminLoginOTP || input !== currentAdminLoginOTP) {
        alert("Kode verifikasi salah. Cek email kamu lagi atau klik \"Kirim Ulang Kode\".");
        return;
    }
    currentAdminLoginOTP = null;
    document.getElementById("adminOtpModal").classList.add("hidden");
    initApp(); // Baru masuk dashboard admin setelah OTP benar
};
