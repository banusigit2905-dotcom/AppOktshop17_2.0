...
function loadActivationList() {
    ...
    unsubActivationUsers = db.collection("users").where("role", "==", "reseller").where("isActive", "==", false)
        .onSnapshot(snap => {
            ...
            if (typeof unsubActivationOrders === 'function') unsubActivationOrders();
            unsubActivationOrders = db.collection("orders").where("status", "==", "Selesai").onSnapshot(sOrders => {
                const allOrders = sOrders.docs.map(d => d.data());
                document.getElementById("adminActivationTable").innerHTML = pendingUsers.map(u => {
                    const totalBelanja = allOrders.filter(o => o.resellerId === u.id).reduce((s, o) => s + (o.total || 0), 0);
                    return `<tr><td>${u.customId}</td><td>${u.nama}</td><td>${u.email}</td><td>Rp ${totalBelanja.toLocaleString('id-ID')}</td><td><button class="btn-adm-action" onclick="activateUser('${u.id}')">AKTIFKAN</button></td></tr>`;
                }).join('') || `<tr><td colspan="5" style="text-align:center;">Tidak ada akun baru untuk diaktivasi</td></tr>`;
            });
        });
}
