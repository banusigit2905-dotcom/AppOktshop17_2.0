...
function loadActivationList() {
    ...
    unsubActivationUsers = db.collection("users").where("role", "==", "reseller")
        .onSnapshot(snap => {
            ...
            const msg = `<tr><td colspan="5" style="text-align:center;color:red;">Gagal memuat data: ${err.message}</td></tr>`;
            document.getElementById("adminActivationTable").innerHTML = msg;
            document.getElementById("adminMemberListTable").innerHTML = msg;
        });
...
function renderMemberListTable() {
    ...
    if (_actAllResellers.length === 0) {
        tbody.innerHTML = `<tr><td colspan="5" style="text-align:center;">Belum ada member reseller</td></tr>`;
    ...
    tbody.innerHTML = pageItems.map(u => {
        ...
        return `<tr><td>${u.customId}</td><td>${u.nama}</td><td>${u.email}</td><td>${statusBadge}</td><td>${_hitungBelanja(u.id)}</td></tr>`;
    }).join('');
