...
const REFERRAL_BASE_POINTS = 5000; // Poin ke pengundang, cair saat reseller baru belanja pertama kali >= syarat minimal
const REFERRAL_MIN_ORDER = 50000;  // Syarat minimal belanja (Rp) supaya poin referral cair
...
// ---------------------------------------------------------------
// 1) PEMBAYARAN POIN DASAR — dipanggil dari script.js setelah checkout berhasil.
//    Poin BELUM cair saat aktivasi akun; baru cair begitu reseller baru yang
//    didaftarkan pakai kode referral berhasil checkout dengan total >= Rp 50.000.
// ---------------------------------------------------------------
async function processReferralQualifyingOrder(newUserId, orderTotal) {
    if (!orderTotal || orderTotal < REFERRAL_MIN_ORDER) return; // belum memenuhi syarat minimal belanja
    ...
    if (nextMilestone) {
        ...
        progressHtml = `
            <div class="next-milestone">
                ...
                <div class="hint">Kamu sudah melewati semua tangga bonus referral (${REFERRAL_LADDER[REFERRAL_LADDER.length - 1].count} reseller). Tetap dapat <b>${REFERRAL_BASE_POINTS.toLocaleString('id-ID')} poin</b> setiap referral baru yang belanja pertama minimal Rp ${REFERRAL_MIN_ORDER.toLocaleString('id-ID')}.</div>
            </div>`;
    }
...
function buildReferralTableHTML() {
    ...
    return sorted.map(r => {
        ...
        return `<div class="ref-row">
                    ...
                    <span class="status-tag ${isActive ? 'status-active' : 'status-pending'}">${isActive ? 'Poin Cair' : 'Menunggu Belanja'}</span>
                </div>`;
    }).join('');
}
...
function renderReferralUI() {
    ...
    // --- Ringkasan mini di Dashboard Reseller ---
    const ringEl = document.getElementById("dashRefRing");
    const ringTxtEl = document.getElementById("dashRefRingTxt");
    const textEl = document.getElementById("dashRefText");
    if (ringEl && ringTxtEl && textEl) {
        const nextMilestone = REFERRAL_LADDER.find(m => activeCount < m.count);
        const target = nextMilestone ? nextMilestone.count : REFERRAL_LADDER[REFERRAL_LADDER.length - 1].count;
        const pct = Math.min(1, activeCount / target);
        const circumference = 132; // 2 * PI * r(21), dibulatkan sesuai atribut SVG
        ringEl.setAttribute("stroke-dashoffset", String(circumference * (1 - pct)));
        ringTxtEl.textContent = `${activeCount}/${target}`;
        textEl.textContent = nextMilestone
            ? `${nextMilestone.count - activeCount} lagi menuju bonus ${nextMilestone.bonus.toLocaleString('id-ID')} poin`
            : `Semua tangga tercapai! Tetap dapat ${REFERRAL_BASE_POINTS.toLocaleString('id-ID')} poin/referral`;
    }
    ...
    container.innerHTML = `
        ...
        <div class="summary-row">
            ...
            <div class="summary-pill sp-active"><b>${activeCount}</b>Poin Cair</div>
