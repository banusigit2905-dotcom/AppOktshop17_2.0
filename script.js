...
function setRunningText(text) {
    ...
    // Sinkronkan juga ke banner bold-flat di Dashboard Reseller (kalau elemennya ada)
    const elDash = document.getElementById("runningTextDash");
    if (elDash) {
        elDash.style.animation = "none";
        void elDash.offsetWidth;
        elDash.innerText = text;
        elDash.style.animation = `marquee ${durationSec}s linear infinite`;
    }

    if (!el && !elDash) return 15;
    return durationSec;
}
