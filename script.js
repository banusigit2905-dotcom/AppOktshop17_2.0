    ...
    db.collection("redemptions").onSnapshot(snap => {
        ...
        if (tableBody) {
            tableBody.innerHTML = snap.docs.map(d => {
                ...
                return `<tr><td><b>${r.resellerName}</b></td><td>${r.points.toLocaleString()}</td><td><span class="badge-selesai">Selesai</span></td></tr>`;
            }).join('');
        }
        const pending = snap.docs.filter(d => d.data().status === 'proses').length;
...
document.getElementById("orderFormFinal").onsubmit = async (e) => {
    ...
    try {
        ...
        const waText = `*PESANAN BARU*%0AOrder: ${orderNo}%0APenerima: ${cust}%0AMetode Pembayaran: ${pay}%0AProduk:%0A${detailWA}%0ATotal: Rp ${total.toLocaleString()}`;
        closeOrderModal();
...
document.getElementById("formRedeemPoints").onsubmit = async (e) => {
    ...
    try {
        ...
        await db.collection("redemptions").add({
            ...
            namaPenerima: np, whatsapp: wp, status: "proses", otpCode: currentRedeemOTP,
            kode: redeemCode,
            createdAt: firebase.firestore.FieldValue.serverTimestamp()
        });
        ...
        closeRedeemModal();
        window.open(`https://wa.me/62895345452412?text=Penukaran Poin ${amt} - ${np} (OTP: ${otpInput})`, '_blank');
...
function loadActivationList() {
    ...
    // Query user & query orders dibuat TERPISAH (independen) supaya daftar user
    // tetap tampil walau query orders (untuk kolom Belanja) gagal/lambat.
    unsubActivationUsers = db.collection("users").where("role", "==", "reseller").where("isActive", "==", false)
        .onSnapshot(snap => {
            _actPendingUsers = snap.docs.map(doc => ({ id: doc.id, ...doc.data() }));
            renderActivationTable();
        }, err => {
            console.error("Gagal memuat daftar aktivasi:", err);
            document.getElementById("adminActivationTable").innerHTML =
...
function renderActivationTable() {
    ...
    if (!tbody) return;
    if (_actPendingUsers.length === 0) {
        tbody.innerHTML = `<tr><td colspan="5" style="text-align:center;">Tidak ada akun baru untuk diaktivasi</td></tr>`;
        return;
    }
    tbody.innerHTML = _actPendingUsers.map(u => {
        const totalBelanja = _actAllOrders.filter(o => o.resellerId === u.id).reduce((s, o) => s + (o.total || 0), 0);
        const belanjaText = _actOrdersLoaded ? `Rp ${totalBelanja.toLocaleString('id-ID')}` : '...';
        return `<tr><td>${u.customId}</td><td>${u.nama}</td><td>${u.email}</td><td>${belanjaText}</td><td><button class="btn-adm-action" onclick="activateUser('${u.id}')">AKTIFKAN</button></td></tr>`;
    }).join('');
}
