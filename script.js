...
document.getElementById("registerForm").onsubmit = async (e) => {
    e.preventDefault();
    alert("DEBUG 1: Tombol berhasil diklik, form submit terdeteksi.");

    // Validasi field standar dulu (nama, email, sandi, hp)
    if (!e.target.checkValidity()) {
        alert("DEBUG 2A: Ada field yang belum valid/kosong.");
        e.target.reportValidity();
        return;
    }
    alert("DEBUG 2B: Semua field valid, lanjut ke pengecekan privasi.");
    ...
    alert("DEBUG 3: agreeCheckbox ditemukan? " + (agreeCheckbox ? "YA" : "TIDAK") + " | checked: " + (agreeCheckbox ? agreeCheckbox.checked : "-"));
    if (agreeCheckbox && !agreeCheckbox.checked) {
        window.pendingRegistrationAfterPrivacy = true;
        alert("DEBUG 4: openPrivacyModal ada? " + (typeof openPrivacyModal === 'function' ? "YA, akan dibuka" : "TIDAK ADA - INI MASALAHNYA"));
    ...
    alert("DEBUG 5: Lanjut ke performRegistration()...");
...
async function performRegistration() {
    ...
    try {
        // Kalau ada kode referral diisi, cari dulu pemilik kode-nya SEBELUM bikin akun
        let referrer = null;
        if (refCode) {
...
async function activateUser(uid) {
    if(confirm("Aktifkan?")) {
        ...
        // Bayar poin referral ke pengundang (kalau akun ini didaftarkan pakai kode referral)
        if (typeof processReferralActivation === 'function') {
            processReferralActivation(uid).catch(err => console.log("Referral payout gagal:", err.message));
        }
...
function renderSidebar() {
    ...
    if (currentUser.role === 'admin') {
        ...
        menuItems = `
            ...
            <div class="nav-item" onclick="showSection('secReferral')">🔗 Referral</div>
