...
function loadAdminData() {
    ...
    db.collection("redemptions").onSnapshot(snap => {
        ...
        if (tableBody) {
            tableBody.innerHTML = snap.docs.map(d => {
                const r = d.data();
                const ewLabel = r.ewallet ? `<b>${r.ewallet}</b>` : '-';
                if (r.status === 'proses') {
                    return `<tr>
...
document.getElementById("formRedeemPoints").onsubmit = async (e) => {
    ...
    try {
        ...
        await db.collection("redemptions").add({
            ...
            namaPenerima: np, whatsapp: wp, ewallet: ew, status: "proses", otpCode: currentRedeemOTP,
            kode: redeemCode,
            createdAt: firebase.firestore.FieldValue.serverTimestamp()
        });
        ...
        closeRedeemModal();
        window.open(`https://wa.me/62895345452412?text=Penukaran Poin ${amt} - ${np} (${ew}) (OTP: ${otpInput})`, '_blank');
