...
function startAdminOtpVerification() {
    ...
    if (!modal) {
        alert("⚠️ Bagian verifikasi admin tidak ditemukan di halaman ini. Kemungkinan file aplikasi belum ter-update sepenuhnya. Coba hapus cache/data situs, lalu buka ulang.");
        auth.signOut();
        return;
    }
    modal.classList.remove("hidden");
...
function buildAdminTotp(secretBase32) {
    if (typeof OTPAuth === "undefined") {
        throw new Error("Library OTPAuth gagal dimuat (koneksi internet bermasalah saat membuka halaman). Coba refresh halaman dan login ulang.");
    }
...
function generateAdminTotpSecret() {
    if (typeof OTPAuth === "undefined") {
        alert("⚠️ Library keamanan (OTPAuth) gagal dimuat dari server. Ini biasanya karena koneksi internet lambat/putus saat halaman dibuka.\n\nCoba: tutup aplikasi, pastikan internet stabil, lalu buka & login lagi.");
        auth.signOut();
        return;
    }
...
function renderSidebar() {
    ...
    menuItems += `<div class="nav-item" onclick="showSection('secFaq')">❓ Pusat Bantuan</div>`;
...
function closeLogoutConfirm() { document.getElementById("logoutConfirmModal").classList.add("hidden"); }
function doLogout() { closeLogoutConfirm(); stopPresenceHeartbeat(); auth.signOut(); }
...
function switchAuth(mode) {
    ...
    if (mode === 'login') {
        ...
        if (sheetTitle) sheetTitle.textContent = "Masuk";
        if (sheetDesc) sheetDesc.textContent = "Masuk pakai akun reseller kamu, atau daftar dulu kalau belum punya akun.";
    } else {
        ...
        hideRegIpWarning();
        if (sheetTitle) sheetTitle.textContent = "Daftar";
        if (sheetDesc) sheetDesc.textContent = "Buat akun reseller baru, isi data di bawah ini dengan lengkap.";
    }
}
...
// --- PUSAT BANTUAN (FAQ) — pencocokan kata kunci otomatis, tanpa API luar ---
const faqData = [
    {
        keywords: ["daftar", "registrasi", "buat akun", "gabung reseller"],
        q: "Bagaimana cara mendaftar jadi reseller?",
        a: "Di halaman login, pilih tab <b>DAFTAR</b>, isi nama, email, kata sandi, dan nomor WhatsApp. Setelah klik \"Daftar & Aktivasi\", kamu akan diarahkan chat WhatsApp ke admin untuk aktivasi akun."
    },
    {
        keywords: ["aktivasi", "belum aktif", "akun belum bisa", "isactive", "tidak bisa masuk setelah daftar"],
        q: "Kenapa akun saya belum bisa dipakai setelah daftar?",
        a: "Akun baru butuh <b>aktivasi manual dari admin</b>. Setelah daftar, kamu akan diarahkan chat WA ke admin — pastikan itu terkirim. Admin akan aktifkan akunmu secepatnya."
    },
    {
        keywords: ["lupa password", "lupa sandi", "reset password", "ganti password", "kata sandi lupa"],
        q: "Saya lupa password, bagaimana cara reset?",
        a: "Di halaman login, klik <b>\"Lupa Password? Reset di sini\"</b> di bawah tombol Masuk. Masukkan email kamu, cek inbox (atau folder spam) untuk link reset password dari Firebase."
    },
    {
        keywords: ["pesan produk", "order", "cara beli", "checkout", "belanja", "kirim pesanan"],
        q: "Bagaimana cara memesan produk?",
        a: "Buka menu produk, pilih barang & jumlahnya, lalu isi <b>Detail Pengiriman</b> (nama penerima, no HP, jenis pembayaran, alamat kalau Transfer/QRIS, dan jam kirim). Klik \"Konfirmasi & Kirim WhatsApp\" untuk kirim pesanan ke admin."
    },
    {
        keywords: ["tukar poin", "redeem", "poin ditukar", "cairkan poin", "e-wallet", "dana", "gopay", "shopeepay"],
        q: "Bagaimana cara menukar poin?",
        a: "Buka menu <b>Tukar Poin</b>, pilih nominal, isi data diri, pilih e-wallet tujuan (Dana/GoPay/ShopeePay) beserta nomor HP-nya, lalu verifikasi kode OTP yang dikirim ke email kamu."
    },
    {
        keywords: ["dapat poin", "sumber poin", "checklist", "poin harian", "poin mingguan", "bonus poin"],
        q: "Dari mana saya mendapatkan poin?",
        a: "Poin didapat dari <b>Checklist Mingguan</b> — isi checklist harian selama 7 hari berturut-turut buat dapat bonus poin tambahan. Poin harian tetap aman tersimpan walau ada hari yang terlewat."
    },
    {
        keywords: ["retur", "kembalikan barang", "barang rusak", "tukar barang"],
        q: "Bagaimana cara retur barang?",
        a: "Buka menu <b>Retur Barang</b> di sidebar, isi form retur dengan alasan dan detail barangnya. Admin akan memproses dan menghubungimu lewat WhatsApp."
    },
    {
        keywords: ["komplain", "keluhan", "laporan masalah", "lapor"],
        q: "Bagaimana cara melapor keluhan/masalah?",
        a: "Buka menu <b>Laporan Keluhan</b> di sidebar, isi form-nya dengan jelas. Admin akan menindaklanjuti keluhanmu secepatnya."
    },
    {
        keywords: ["referral", "kode referral", "ajak teman", "undang teman"],
        q: "Apa itu kode referral?",
        a: "Kode referral bisa dibagikan ke calon reseller lain. Kalau mereka daftar pakai kode referral kamu, biasanya ada bonus/poin tambahan buat kamu — cek detail bonusnya ke admin."
    },
    {
        keywords: ["otp", "kode verifikasi", "google authenticator", "2fa", "verifikasi 2 langkah", "email otp"],
        q: "Kode OTP/verifikasi tidak masuk, kenapa?",
        a: "Cek folder <b>spam/promosi</b> di email kamu. Kalau masih belum ada, klik \"Kirim Ulang Kode\" di halaman verifikasi. Kalau tetap gagal, hubungi admin lewat WhatsApp."
    },
    {
        keywords: ["ubah profil", "ganti nama", "update data", "ganti nomor hp", "edit profil"],
        q: "Bagaimana cara mengubah data profil?",
        a: "Buka menu <b>Profil Akun</b> di sidebar, ubah nama atau nomor HP-mu, lalu klik UPDATE."
    },
    {
        keywords: ["hubungi admin", "kontak admin", "cs", "customer service", "whatsapp admin"],
        q: "Bagaimana cara menghubungi admin langsung?",
        a: "Kamu bisa chat admin langsung lewat WhatsApp di <a href=\"https://wa.me/62895345452412\" target=\"_blank\" style=\"color:#C62828;font-weight:700;\">wa.me/62895345452412</a>."
    }
];

function searchFaq(query) {
    const q = (query || "").toLowerCase().trim();
    const resultsBox = document.getElementById("faqResults");
    if (!resultsBox) return;

    let scored = faqData.map(item => {
        let score = 0;
        if (q === "") {
            score = 1; // tampilkan semua kalau belum ketik apa-apa
        } else {
            item.keywords.forEach(k => { if (q.includes(k) || k.includes(q)) score++; });
            if (item.q.toLowerCase().includes(q)) score += 2;
        }
        return { ...item, score };
    ...
    if (scored.length === 0) {
        resultsBox.innerHTML = `
            <div class="faq-empty">
                <p style="font-size:13px; margin-bottom:10px;">😅 Belum ketemu jawabannya nih. Coba kata kunci lain, atau chat admin langsung:</p>
                <a href="https://wa.me/62895345452412" target="_blank" class="btn-primary" style="display:block; text-align:center; text-decoration:none;">💬 Chat Admin via WhatsApp</a>
            </div>`;
        return;
    }

    resultsBox.innerHTML = scored.map((item, i) => `
        <div class="faq-item">
            <div class="faq-q" onclick="toggleFaqAnswer(${i})">
                <span>${item.q}</span><span class="faq-caret" id="faqCaret${i}">▾</span>
            </div>
            <div class="faq-a hidden" id="faqAnswer${i}">${item.a}</div>
        </div>
    `).join("");
}

function toggleFaqAnswer(i) {
    const ans = document.getElementById("faqAnswer" + i);
    const caret = document.getElementById("faqCaret" + i);
    if (!ans) return;
    ans.classList.toggle("hidden");
    caret.textContent = ans.classList.contains("hidden") ? "▾" : "▴";
}
